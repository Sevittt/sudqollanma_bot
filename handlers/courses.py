...
def get_modules_keyboard(course):
    ...
    buttons = [
        [InlineKeyboardButton(text=f"{i+1}. {m['title']}", callback_data=f"module_{course['id']}_{i}")]
...
def get_lessons_keyboard(course_id, module, m_idx):
    ...
    type_icons = {'video': '📹', 'article': '📄', 'pdf': '📂', 'quiz': '🧠'}
    buttons = [
        [InlineKeyboardButton(
            text=f"{type_icons.get(l['type'], '📌')} {l['title']}",
            callback_data=f"lesson_{course_id}_{m_idx}_{i}"
        )]
...
async def show_lesson_content(callback: CallbackQuery):
    ...
    type_icons = {'video': '📹', 'article': '📄', 'pdf': '📂', 'quiz': '🧠'}
    icon = type_icons.get(lesson['type'], '📌')
    ...
    if lesson['type'] == 'video':
        text += f"🎬 Videoni ko'rish uchun ID:\n<code>{lesson['refId']}</code>"
    elif lesson['type'] == 'article':
        text += "📝 Maqolani o'qish uchun 'Maqolalar' bo'limidan foydalaning."
    elif lesson['type'] == 'pdf':
        text += "📂 Faylni 'Qo'llanmalar' bo'limidan yuklab oling."
    elif lesson['type'] == 'quiz':
        text += "🧠 Testni 'Test ishlash' bo'limidan boshlang."
