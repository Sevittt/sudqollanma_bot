...
def get_main_menu():
    """Main menu keyboard — BotFather buyruqlari bilan to'liq mos."""
...
async def process_competency_answer(message: Message, state: FSMContext, current_idx: int, next_state):
    ...
    if next_state:
        ...
        if not court_name:
            # Sud hali tanlanmagan — court selection oqimiga yo'naltiramiz
            await message.answer(
...
# =============================================
# COURT SELECTION FLOW (Flutterga o'xshash)
# =============================================
...
async def show_region_selection(message: Message, state: FSMContext):
    """Foydalanuvchiga viloyat/shahar tanlashni ko'rsatish."""
    try:
        from services.firestore_service import FirestoreService
        # Firestore courts kolleksiyasidan unique regionlarni olish
...
async def handle_region_selection(message: Message, state: FSMContext):
    """Viloyat tanlangandan so'ng sud turini ko'rsatish."""
...
async def handle_court_type_selection(callback: CallbackQuery, state: FSMContext):
    """Sud turi tanlangandan so'ng aniq sud nomlarini ko'rsatish."""
    ...
    if not courts:
        # Agar sud topilmasa — nomni qo'lda kiritsin
    ...
    # Sudlar ro'yxatini keyboard sifatida ko'rsatish (2 ustunli agar 6 dan ko'p)
    if len(courts) <= 8:
...
async def handle_court_selection(message: Message, state: FSMContext):
    """Sud nomi kiritilgandan so'ng lavozim tanlashni ko'rsatish."""
...
async def handle_position_selection(message: Message, state: FSMContext):
    """Lavozim kiritilgandan so'ng barcha ma'lumotlarni Firestorega saqlash."""
    data = await state.get_data()
...
async def set_role_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if not user:
        ...
        if not court_name:
            # Sud hali tanlanmagan — court selection ga yo'naltiramiz
            await callback.message.answer(
                f"✅ Lavozimingiz <b>{friendly_name}</b> etib belgilandi!\n\n"
                f"Endi qaysi sudda ishlashingizni belgilaylik:",
                parse_mode="HTML"
