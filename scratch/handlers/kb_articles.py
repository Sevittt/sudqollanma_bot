"""
Handler: /maqolalar — Bilimlar bazasi va maqolalar
Firestore 'knowledge_base' kolleksiyasidan maqolalar ko'rsatadi.
/faq — Firestore 'faqs' kolleksiyasidan savollar.
"""
...
async def _get_knowledge_articles(category: str = None, limit: int = 20) -> list:
    """'knowledge_base' kolleksiyasidan maqolalar olish."""
    if not get_db():
        return []
    try:
...
async def _get_faqs(category: str = None, limit: int = 20) -> list:
    """'faqs' kolleksiyasidan savollar olish."""
    if not get_db():
        return []
    try:
...
def _build_category_keyboard(prefix: str):
    ...
    buttons.append([
        InlineKeyboardButton(text="📋 Barchasi", callback_data=f"{prefix}_cat_all")
    ])
...
async def maqolalar_command(message: Message):
    """Bilimlar bazasidan maqolalar ko'rsatish."""
...
async def handle_kb_category(callback: CallbackQuery):
    """Tanlangan kategoriya bo'yicha maqolalar ko'rsatish."""
    ...
    if not articles:
        cat_name = CATEGORY_NAMES.get(cat, "Barchasi")
        buttons = [[InlineKeyboardButton(text="⬅️ Orqaga", callback_data="kb_back")]]
        await callback.message.edit_text(
            f"📭 Bu bo'limda hozircha maqolalar mavjud emas.",
...
async def handle_kb_read(callback: CallbackQuery):
    """Maqola to'liq matnini ko'rsatish."""
    ...
    try:
        ...
        # Content truncation for Telegram limit
        ...
        if pdf_url:
            buttons.append([
                InlineKeyboardButton(text="📥 PDF yuklab olish", url=pdf_url)
            ])
        buttons.append([InlineKeyboardButton(text="⬅️ Orqaga", callback_data="kb_back")])
...
async def handle_faq_category(callback: CallbackQuery):
    """Tanlangan kategoriya bo'yicha FAQlar."""
    ...
    if not faqs:
        await callback.message.edit_text(
            f"📭 Bu bo'limda hozircha savollar mavjud emas.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    buttons = [[InlineKeyboardButton(text="⬅️ Orqaga", callback_data="faq_back")]]
